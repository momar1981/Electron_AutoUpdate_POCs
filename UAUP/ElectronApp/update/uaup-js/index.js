//https://github.com/DcmanProductions/UAUP-Example
////////////////////    IMPORTING    ////////////////////
//#region IMPORTING
...
function setupGitProtocol(options) {
    ...
    git_api = options.gitRepo + options.appName;//`https://api.github.com/repos/${options.gitUsername}/${options.gitRepo}/releases/latest`;
}
...
async function GetUpdateURL(options) {
    ...
    /*return fetch(git_api).then(response => response.json()).then(data => { json = data; }).catch(e => {
        try {
            ...
            // NodeJS
            console.error(`GetUpdateURL >> Something went wrong: ${e}`);
            return;
        }
    }).then(() => {
        let zip;
...
async function GetUpdateVersion(options) 
{
    ...
    .catch(e => {
        try {
            // Electron
            console.log(`GetUpdateVersion >> Something went wrong: ${e}`);
        } catch {
            // NodeJS
            console.error(`GetUpdateVersion >> Something went wrong: ${e}`);
            return;
        }
    }).then(() => {
...
async function Update(options = defaultOptions) {
    if (testOptions(options)) {
        ...
        try {
            // Electron
            console.log(`Update >> Please Fill out the Options Fully`);
            window.close();
        } catch {
            // NodeJS
            console.log(`Update >> Please Fill out the Options Fully`);
...
async function CheckForUpdates(options = defaultOptions) {
    if (testOptions(options)) {
        ...
        if (fs.existsSync(options.versionFile))//not first time to open the app at this pc , so may be we need to update it .. 
        {
        ...
        else //first time to open the app after download it from the web , so we do not need to update it
        {
            console.warn("Couln't find the Version File, this usually means that there was no previous update.")
            UpdateCurrentVersion(options);//Just we need to create the file to be indecator for next time we open the app 
            return false;
        }
    }
}
...
function Install(options) {
    ...
    setTimeout(() => CleanUp(options), 10000);
}
...
function LaunchApplication(options) {
   ...
   setTimeout(() => {
        ...
        {
            // Electron
            console.log("electron window.colse ...");
            window.close();
        } 
        catch (e) 
        {
            console.log("electron window.colse has error :");
            console.error(e);
            // NodeJS
            try
            {     
                console.log("NodeJS colse proccess ...");
                process.exit(0);
            }
            catch (ex) 
            {
                console.log("NodeJS colse proccess  has error :");
                console.error(ex);
            }
        }
    }, 1000);
